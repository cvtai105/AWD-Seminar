...
const FacebookOauth2RedirectHandler = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const { login } = useAppContext();

  useEffect(() => {
    // Lấy các tham số từ URL callback (authorization code, state, etc.)
    ...
    if (authorizationCode) {
      ...
      instance
        .get(`${import.meta.env.VITE_FACEBOOK_GET_ACCESS_TOKEN_URL}`, {
          params: {
        ...
        .then((response) => {
          instance
            .get(`${import.meta.env.VITE_USER_INFO}`, {
              params: {
...
export default FacebookOauth2RedirectHandler;
